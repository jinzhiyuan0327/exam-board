...
export type FlushResult =
  ...
  | { kind: 'offline' | 'error' | 'unauthorized' };
...
export async function flushPendingExamSync(): Promise<FlushResult> {
  ...
  if (first == null || !first.remote) return { kind: 'error' };
  ...
  if (typeof retry !== 'number') return retry === 'unauthorized' ? { kind: 'unauthorized' } : { kind: 'error' };
